...
function getPageParams(q: any) {
  // Accept BOTH pageSize and limit (alias), prefer limit if present
  ...
  return { hasPaging, page, limit, pageSize: limit, skip, take };
}
...
function pageMeta(total: number, page: number, limit: number) {
  ...
  return {
    ...
    page,
    limit,           // ✅ preferred key
    pageSize: limit, // ✅ backward-compat for any existing UI
    pages,
    hasNext: page < pages,
    hasPrev: page > 1,
  };
...
r.post("/", writeLimiter, validateBody(zStockMovementCreate), async (req, res) => {
  ...
  if (direction === "OUT") {
    // Decrease path — guard with lock + transaction
    try {
      ...
      // If we responded inside the transaction (insufficient_stock), stop
      if ((created as any)?.json) return;
      return res.status(201).json(created);
    } catch (e: any) {
      return res.status(500).json({ error: "failed_to_create_stock_movement", detail: e?.message });
    }
  }

  // IN path — no guard required
  try {
